...
//Controller Divisi Penghimpunan dan Pendayagunaan Peminjaman Ruang Rapat
...
class Penghimpunan extends CI_Controller{
	public function __construct()
	{
		...
		$this->load->model('ruangrapat/Penghimpunan_model2');
	}
	...
	public function index()
	{
		...
		$data['jumlahuser'] = $this->Penghimpunan_model2->get_username();
		$data['jumlahbooking'] = $this->Penghimpunan_model2->get_booking();
		$this->load->view('template/v_header2',$data);
		$this->load->view('ruangrapat/Home/beranda_penghimpunan',$data);  
	...
	public function penghimpunan()
	{
		...
		$data['booking'] = $this->Penghimpunan_model2->getAllbooking($this->session->userdata('id_divisi'));
		$data['divisi'] = $this->Penghimpunan_model2->get_member_by_id($this->session->userdata('id_divisi'));
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->load->view('template/v_header2',$data);
			$this->load->view('ruangrapat/penghimpunan/form',$data);
			$this->load->view('template/v_footer');
		}
		else{
			$this->Penghimpunan_model2->tambahDataBooking();
			$this->session->set_flashdata('flash','Ditambahkan');
			redirect('ruangrapat/penghimpunan/confirmasi');
		}
	...
	{
		$data['judul'] = 'Halaman Ruang Rapat';
		$data['user'] = $this->Penghimpunan_model2->get_user();
		$data['booking'] = $this->Penghimpunan_model2->getAllbooking($this->session->userdata('id_divisi'));
		$data['divisi'] = $this->Penghimpunan_model2->get_divisi();
		
		$this->load->view('template/v_header2',$data);
		$this->load->view('ruangrapat/penghimpunan/table',$data);
		$this->load->view('template/v_footer');
	...
	public function penghimpunansemua()
	{
		$data['judul'] = 'Halaman Ruang Rapat';
		$data['user'] = $this->Penghimpunan_model2->get_user();
		$data['booking'] = $this->Penghimpunan_model2->getAllSemuaBooking();
		$data['divisi'] = $this->Penghimpunan_model2->get_divisi();
		$this->load->view('template/v_header2',$data);
		$this->load->view('ruangrapat/penghimpunan/semua',$data);
		$this->load->view('template/v_footer');
	...
	public function detail($id)
	{
		...
		$data['detail'] = $this->Penghimpunan_model2->getBookingById($id);
		$this->load->view('template/v_header2',$data);
		$this->load->view('ruangrapat/penghimpunan/detail',$data);
		$this->load->view('template/v_footer');
	}
	...
	public function confirmasi()
	{
       ...
       // successfully sent mail to user email  
			$this->session->set_flashdata('flash','Ditambahkan');  
			redirect(base_url('ruangrapat/penghimpunan/table'));  
		}  
		else  
		{  
		...
		public function sendemail(){
			...
			if (!$this->email->send()) {  
				...
				$this->session->set_flashdata('flash','Ditambahkan');  
				redirect('ruangrapat/penghimpunan/table');   
			}  
		}
		...
		public function edit($id){
			...
			$data['edit'] = $query->row();
			$this->load->view('template/v_header2',$data);
			$this->load->view('ruangrapat/penghimpunan/form_ubah',$data);
			$this->load->view('template/v_footer');
		}
		...
		public function ubah($id)
		{
			...
			$data['hari'] = ['Senin','Selasa','Rabu','Kamis','Jumat','Sabtu','Minggu'];
			$data['user'] = $this->session->userdata('nama_lengkap');
			$data['booking'] = $this->Penghimpunan_model2->getBookingById($id);
