...
//Controller Divisi Kepatuhan Hukum dan SDM Peminjaman Mobil
...
class kepatuhan extends CI_Controller{
	...
	{
		parent ::__construct();
		$this->load->model('mobil/kepatuhan_model');
		$this->load->helper('form');
		$this->load->library('form_validation');
		$this->load->library('session');
		$this->load->helper('url');

		if ($this->session->userdata('id_divisi')!="4") 
		{
	...
	public function index()
	{
		...
		$data['count']=$this->kepatuhan_model->get_count_booking();
		$data['count_m']=$this->kepatuhan_model->get_count_mobil();
		$data['jumlahuser'] = $this->kepatuhan_model->get_username();
		$data['jumlahbooking'] = $this->kepatuhan_model->get_booking();
	...
	public function kepatuhan()
	{
		...
		$data['mobil']= $this->kepatuhan_model->get_mobil();
		$data['booking'] = $this->kepatuhan_model->getAllBooking($this->session->userdata('id_divisi'));
		$data['divisi'] = $this->kepatuhan_model->get_member_by_id($this->session->userdata('id_divisi'));
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->load->view('template/v_header5',$data);
			$this->load->view('mobil/kepatuhan/v_form',$data);
			$this->load->view('template/v_footer');
		}
		else{
			$this->kepatuhan_model->tambahDataBooking();
			$this->session->set_flashdata('flash','Ditambahkan');
			redirect('mobil/kepatuhan/confirmasi');
		}
	...
	public function confirmasi()
	{
       ...
       // successfully sent mail to user email  
			$this->session->set_flashdata('flash','Ditambahkan');  
			redirect(base_url('mobil/kepatuhan/table'));  
		}  
		else  
		{  
		...
		public function sendemail(){
			...
			$this->email->message("<html><head><head></head><body><p><b>Hai Admin!</b></p>Divisi Kepatuhan dan SDM baru saja membooking mobil YBM PLN Gandaria.<p> Klik <strong><a href='http://192.168.2.110/ci_mobil/admin/databooking' target='_blank' rel='noopener' class='btn btn-danger'>disini</a></strong> untuk melihat detailnya.</p><br/>Terima Kasih </p></body></html>");  
			if (!$this->email->send()) {  
				show_error($this->email->print_debugger());   
			}
			else{  
				$this->session->set_flashdata('flash','Ditambahkan');  
				redirect('mobil/kepatuhan/table');   
			}  
		}  
		...
		public function detail($id)
		{
			...
			$data['detail'] = $this->kepatuhan_model->getBookingById($id);
			$this->load->view('template/v_header5',$data);
			$this->load->view('mobil/kepatuhan/v_detail',$data);
			$this->load->view('template/v_footer');
		}
		...
		public function edit($id){
			...
			$data['edit'] = $query->row();
			$this->load->view('template/v_header5',$data);
			$this->load->view('mobil/kepatuhan/v_ubah',$data);
			$this->load->view('template/v_footer');
		}
		...
		public function ubah($id)
		{
			...
			$data['booking'] = $this->kepatuhan_model->getBookingById($id);
			...
			{
				redirect('mobil/kepatuhan/update');
			}
		}
		...
		public function table()
		{
			...
			$data['user'] = $this->kepatuhan_model->get_user();
			$data['booking'] = $this->kepatuhan_model->getAllbooking($this->session->userdata('id_divisi'));
			$data['divisi'] = $this->kepatuhan_model->get_divisi();
			$data['hari'] = ['Senin','Selasa','Rabu','Kamis','Jumat','Sabtu','Minggu'];

			$this->load->view('template/v_header5',$data);
			$this->load->view('mobil/kepatuhan/table',$data);
			$this->load->view('template/v_footer');
		...
		public function kepatuhansemua()
		{
			...
			$data['user'] = $this->kepatuhan_model->get_user();
			$data['booking'] = $this->kepatuhan_model->getAllSemuaBooking();
			$data['divisi'] = $this->kepatuhan_model->get_divisi();
			$data['hari'] = ['Senin','Selasa','Rabu','Kamis','Jumat','Sabtu','Minggu'];

			$this->load->view('template/v_header5',$data);
			$this->load->view('mobil/kepatuhan/semua',$data);
			$this->load->view('template/v_footer');	
		}
